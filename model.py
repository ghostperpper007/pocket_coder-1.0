...
current_embeddings = encoder.encode(source_complex)
proxy_edge_index   = get_edge_index(source_complex)
max_node           = current_embeddings.size(0) - 1
proxy_edge_index   = proxy_edge_index.clamp(max=max_node)

gnn_base_features = None
delta_accum       = torch.zeros(current_embeddings.size(0), emb_dim)
high_h            = torch.zeros(current_embeddings.size(0), emb_dim)
low_h             = torch.zeros(current_embeddings.size(0), emb_dim)
feedback          = torch.zeros(current_embeddings.size(0), emb_dim)

print("Refined Dual-State Update (High: Context, Low: Planning):\n")

for i in range(6):
    if i == 0:
        # FIX 3: token_attn call restored (was accidentally deleted)
        features          = token_attn(current_embeddings)
        high_h, low_h, b_bias, c_bias, delta = reasoner(features, high_h, low_h, feedback)
        # FIX 4: GNN now receives post-reasoner high_h (not pre-reasoner features)
        gnn_base_features = gnn(high_h, proxy_edge_index)
        delta_accum       = (delta_accum * 0.7 + delta * 0.3).clamp(-1, 1)
        logits            = decoder(features, b_bias, c_bias)
        print("Pass 1: GNN Feature Rewrite complete.")
    else:
        high_h, low_h, b_bias, c_bias, delta = reasoner(
            gnn_base_features + delta_accum, high_h, low_h, feedback
        )
        delta_accum = (delta_accum * 0.7 + delta * 0.3).clamp(-1, 1)
        delta_scale = gnn_base_features.norm() / (delta_accum.norm() + 1e-6)
        features    = gnn_base_features + delta_accum * delta_scale * 0.2
        logits      = decoder(features, b_bias, c_bias)

    # logits detached here — correct place to stop the argmax path,
    # while keeping signal_proj in ASTDiagnosticSystem differentiable
